...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "baseball-game-secret-key-change-in-prod")

STATE = {
    # Shared read-only data (loaded once at startup, never mutated per-user)
    "batting": None,
    "pitching": None,
    "people": None,
    "awards_df": None,
    ...
    "output_dir": "./output",
    # Shared player pool for default filters (rebuilt when filters change)
    "player_ids": [],
}
...
# Per-user mutable state stored in Flask session.
# Keys: score_correct, score_total, streak, best_streak, guesses,
#       hints_given, hint_text, revealed, last_correct,
#       current_name, current_type, current_idx, filters, player_ids


def get_user_state():
    """Initialize session defaults if first visit."""
    if "score_correct" not in session:
        ...
        session["player_ids"] = list(STATE["player_ids"])  # copy shared pool
    return session
...
def get_state_dict():
    ...
    return {
        ...
        "revealed": s["revealed"],
        "player_name": s["current_name"] if s["revealed"] else "??? ???",
        "guesses": s["guesses"],
        "hints_given": s["hints_given"],
        "hint_text": s.get("hint_text", ""),
        "last_correct": s.get("last_correct", False),
        "pool_size": len(s["player_ids"]),
        "filters": s["filters"],
        "current_type": s["current_type"],
    }


def rebuild_pool_for_user():
    """Rebuild per-user player pool from their current filters."""
    s = get_user_state()
    f = s["filters"]
    played_in = None
    ...
    ids = pool["playerID"].tolist()
    random.shuffle(ids)
    session["player_ids"] = ids
    session["current_idx"] = 0
    session.modified = True
    print(f"  Filter applied — pool: {len(ids)} players (mode: {f.get('mode', 'batting')})")


def load_next_player():
    """Load the next player and generate their stats image. Uses per-user session."""
    s = get_user_state()
    player_ids = s["player_ids"]
    current_idx = s.get("current_idx", 0)

    if current_idx >= len(player_ids):
        random.shuffle(player_ids)
        session["player_ids"] = player_ids
        current_idx = 0

    pid = player_ids[current_idx]
    session["current_idx"] = current_idx
    session.modified = True

    # Determine batter vs pitcher
    f = s["filters"]
    mode = f.get("mode", "batting")
    ...
    # Use a per-session image filename so concurrent users don't collide
    sid = session.get("_id", os.urandom(8).hex())
    session["_id"] = sid
    img_filename = f"player_{sid}.png"
    img_path = os.path.join(STATE["output_dir"], img_filename)
    ...
    if is_batter:
        ...
        seasons = [s2 for s2 in seasons if s2["AB"] > 0]
        if len(seasons) == 0:
        ...
        seasons = [s2 for s2 in seasons if s2["IP"] > 0]
        if len(seasons) == 0:
            session["current_idx"] = current_idx + 1
            session.modified = True
            return load_next_player()
    ...
    # Store only what we need — NOT the full seasons list
...
def get_current_seasons_for_reveal():
    """Re-fetch seasons for the current player (used only on reveal/give-up)."""
    s = get_user_state()
    pid = s.get("current_pid")
    current_type = s.get("current_type", "batting")
    if not pid:
        return []
    if current_type == "batting":
        _, seasons = get_player_seasons_batting(
            STATE["batting"], STATE["people"],
            STATE["awards_df"], STATE["allstar_df"], pid,
            STATE["appearances_df"], STATE["awards_share_df"]
        )
        return [s2 for s2 in seasons if s2["AB"] > 0]
    else:
        _, seasons = get_player_seasons_pitching(
            STATE["pitching"], STATE["people"],
            STATE["awards_df"], STATE["allstar_df"], pid,
            STATE["awards_share_df"]
        )
        return [s2 for s2 in seasons if s2["IP"] > 0]


@app.route("/")
def index():
    s = get_user_state()
    # First visit: copy shared pool and load a player
    if not s.get("current_name"):
        if not s.get("player_ids"):
            session["player_ids"] = list(STATE["player_ids"])
            session.modified = True
        load_next_player()
...
def stats_image():
    s = get_user_state()
    img_filename = s.get("current_img", "")
    if img_filename:
        img_path = os.path.join(STATE["output_dir"], img_filename)
...
def guess():
    ...
    if correct:
        ...
        session["score_correct"] = s["score_correct"] + 1
        session["score_total"] = s["score_total"] + 1
        session["streak"] = s["streak"] + 1
        session["best_streak"] = max(s["best_streak"], s["streak"] + 1)
        session.modified = True

        seasons = get_current_seasons_for_reveal()
        img_path = os.path.join(STATE["output_dir"], s.get("current_img", "current_player.png"))
        if s["current_type"] == "batting":
            render_stats_image_batting(seasons, img_path, show_name=s["current_name"])
        else:
            render_stats_image_pitching(seasons, img_path, show_name=s["current_name"])
    else:
        session.modified = True
...
def hint():
    ...
    hints_given = s["hints_given"] + 1
    name_parts = s["current_name"].split()

    if hints_given == 1:
        hint_text = " ".join(p[0] + "." for p in name_parts)
    elif hints_given == 2:
        hint_text = name_parts[0] + " " + " ".join(p[0] + "." for p in name_parts[1:])
    else:
        hint_text = name_parts[0] + " " + " ".join(
            p[:max(2, len(p)//2)] + "\u2026" for p in name_parts[1:]
        )

    session["hints_given"] = hints_given
...
def next_player():
    s = get_user_state()
    session["current_idx"] = s.get("current_idx", 0) + 1
    session.modified = True
    load_next_player()
    return jsonify(get_state_dict())
...
def main():
    ...
    # Build the default shared player pool once at startup
    pool = get_player_pool(
        STATE["batting"], STATE["pitching"], STATE["people"],
        mode=DEFAULT_FILTERS["mode"],
        min_years=DEFAULT_FILTERS["min_years"],
        min_pa=DEFAULT_FILTERS["min_pa"],
        min_ip=DEFAULT_FILTERS["min_ip"],
        played_in=None,
    )
    STATE["player_ids"] = pool["playerID"].tolist()
    random.shuffle(STATE["player_ids"])
    print(f"  Default pool: {len(STATE['player_ids'])} players")
